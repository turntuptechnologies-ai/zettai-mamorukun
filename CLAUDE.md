...
- **Module Stats Collector**: モジュール単位で検知イベント数（Severity 別）と起動時スキャン結果（実行時間・スキャンアイテム数・検知問題数・サマリー）を集計する。MetricsCollector が全体統計を扱うのに対し、こちらはモジュール粒度でパフォーマンスボトルネックや不調モジュールの可視化を支援する。スキャン実行時間はリングバッファ（最新 1024 サンプル）に蓄積され、P50/P95/P99 百分位点・最小/最大/平均を nearest-rank 法で算出する。起動時スキャンに加え、`Module::set_module_stats` で注入された `ModuleStatsHandle` を保持するモジュール（file_integrity / process_monitor / package_verify / suid_sgid_monitor / kernel_module / container_namespace / tls_cert_monitor / ssh_key_monitor / systemd_service / network_monitor / pkg_repo_monitor / sudoers_monitor / sshd_config_monitor / dns_monitor / shell_config_monitor / cron_monitor / security_files_monitor / at_job_monitor / capabilities_monitor / pam_monitor / kernel_params / firewall_monitor / mac_monitor / user_account / ntp_config_monitor）は定期スキャンの実行時間もサンプルとして蓄積する。REST API（`/api/v1/stats/modules`、`/api/v1/stats/modules/{name}`）で取得可能。定期サマリーログ出力にも対応。CLI サブコマンド（`zettai-mamorukun module-stats [--module NAME] [--json] [--save-snapshot PATH] [--diff SNAPSHOT]`）で REST API 経由での取得に加え、任意時点のスナップショットを JSON ファイルに保存し、後から現在値との差分（events / scan percentile 差分）をレポートする機能にも対応。スナップショットには保存時刻（`taken_at` / RFC3339 UTC）が記録され、`--diff` レポートでベースライン取得時刻として表示される（`taken_at` が無い旧形式のスナップショットも後方互換で読み込める）。`compute_diff` の delta 算出は `i128` 中間値経由で `i64` の範囲にサチュレートし、`i64::MAX` / `i64::MIN` に丸められた際はモジュール名・メトリック名・current / baseline を含めた `tracing::warn!` を出力して wrap-around を防ぐ。`total_events_delta` の集計も `saturating_add` でオーバーフロー耐性を確保している
...
src/
  ...
  modules/
    ...
    ntp_config_monitor.rs # NTP / 時刻同期設定監視モジュール（inotify リアルタイム検知・chrony ドロップイン監視・refclock 監査・maxchange max -1 / start 過大監査・logbanner / logchange / logdir 監査・logdir 実ディレクトリのメタデータ監査・logdir シンボリックリンク自己検知・logdir 親ディレクトリ（ancestor）symlink 検知・logdir 親ディレクトリ（ancestor）world-writable 検知対応）
